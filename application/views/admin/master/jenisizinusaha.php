          ...
          <div class="row justify-content-center">
                  ...
                  Data di bawah ini adalah data master yang di tampilkan pada form Penyedia.
                        ...
                        <div class="card-header">
                            <h3 class="card-title"><i class="fas fa-database"></i> Master Jenis Izin Usaha</h3>
                        </div>
                            <div class="card-body">
                                <div class="row">
                                        <div class="col-md-9">
                                        <div class="form-group">
                                              <input type="text" class="form-control" id="keterangans" placeholder="Jenis Izin Usaha">
                                        </div>
                                        </div>
                                        <div class="col-md-3">
                                            <button type="submit" class="btn btn-md btn-primary" onclick="add()">
                                           <i class="fa fa-plus"></i> Tambah Jenis Izin Usaha
                                            </button>
                                        </div>
                               </div>
                               <br>
                                        ...
                                        <tbody>
                                          ...
                                          foreach($data as $datas){
                                              ...
                                              <?php if($datas->status == 1){?>
                                                <button onclick="disable(<?= $datas->id_jenis_izin_usaha?>)" class="btn btn-danger"><i class="fa fa-eye-slash"></i></button>
                                              <button onclick="edits(<?= $datas->id_jenis_izin_usaha?> , '<?= $datas->jenis_izin_usaha?>')" data-toggle="modal" class="btn btn-warning" data-target="#edit"><i class="fa fa-edit"></i></button>
                                              <?php }else {?>
                                              <button onclick="enable(<?= $datas->id_jenis_izin_usaha?>)" class="btn btn-primary"><i class="fa fa-eye"></i></button>
                                              <button onclick="edits(<?= $datas->id_jenis_izin_usaha?> , '<?= $datas->jenis_izin_usaha?>')" data-toggle="modal" class="btn btn-warning" data-target="#edit"><i class="fa fa-edit"></i></button>
                                              <?php } ?>
                                              </td>
                                              </tr>
...
<script>
  var id_field='id_jenis_izin_usaha';
  var field='jenis_izin_usaha';
  var table='static_jenis_izin_usaha';
function add(){
